...
void enqueue_relq_dl_task(struct rq *rq, struct task_struct *task)
{
	...
	struct dl_relq *dl_relq = &rq->relq;
	struct rb_node **link = &dl_relq->rb_root.rb_node;
	struct rb_node *parent = NULL;
	struct sched_dl_entity *entry;
	bool leftmost = true;

	BUG_ON(!RB_EMPTY_NODE(&sched_dl_entity->rb_node));
	while (*link)
	{
		parent = *link;
		entry = rb_entry(parent, struct sched_dl_entity, rb_node);
	...
	if (leftmost)
	{
		dl_relq->rb_leftmost = &sched_dl_entity->rb_node;
	}
	rb_link_node(&sched_dl_entity->rb_node, parent, link);
	rb_insert_color(&sched_dl_entity->rb_node, &dl_relq->rb_root);
}
